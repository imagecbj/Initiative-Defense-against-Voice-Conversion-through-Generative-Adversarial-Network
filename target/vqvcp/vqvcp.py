...
class Quantize(nn.Module):
    ...
    def forward(self, input):
        ...
        embed_ind = embed_ind.view(*input.shape[:-1]).detach().cpu().to(hp.device)
...
def load_model(checkpoint_path):
    ...
    model = VC_MODEL(in_channel=80, channel=64, n_embed=64).to(hp.device)
    print("Loading checkpoint '{}'".format(checkpoint_path))
    checkpoint_dict = torch.load(checkpoint_path, map_location=torch.device(hp.device))
